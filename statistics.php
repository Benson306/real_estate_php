...
<div class='tabs'>
    ...
    <div class='panel8'>
    <h4 id='ribbon' class='well well-sm'><center><font color='black'>Check Statistics From Which Apartment?</font></center></h4>
        <?php 
        $sql = "SELECT * FROM property";
        $run = mysqli_query($conn, $sql);
        while($rows= mysqli_fetch_assoc($run)){
                            echo "<div class='houses'>";
                            echo "<a href='statistics.php?list_id=$rows[property_id]' style='color: black;' class='btn btn-warning btn-block'>$rows[name] Apartment</a>";
                            echo "<br>";
                            echo "<br>";
                            echo "</div>";
        };
...
if(isset($_GET['list_id'])){
    ...
    <script>window.location="statisticsPDF.php"</script>
    <?php
}
